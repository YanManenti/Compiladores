...
def handle_semantic_action(production, tokens, i):
    if production.startswith("const"):
        ...
        if i+1 < len(tokens) and tokens[i+1][0] == 'ident':
            ...
            symbol_table.insert(name=nome, category="constante", type_=tipo, line=linha)
        ...
        while j < len(tokens) and tokens[j][0] != ':':
            if tokens[j][0] == 'ident':
                ...
                tipo = "inteiro" if tipo_token == "integer" else ("real" if tipo_token == "real" else "string")
                symbol_table.insert(name=nome, category="variável", type_=tipo, line=linha)
            j += 1
        ...
        if i+1 < len(tokens) and tokens[i+1][0] == 'ident':
            nome = tokens[i+1][0]
            linha = tokens[i+1][2]
            symbol_table.insert(name=nome, category="procedure", type_="-", line=linha)
